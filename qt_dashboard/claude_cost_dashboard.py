...
A desktop dashboard that:
  ...
  2. Calls `claude -p` to analyze the repo and SUGGEST a set of typical
     tasks tailored to that codebase (e.g. "add input validation to the
     REST handlers", "write integration tests for the parser").
  3. Lets you pick which suggested tasks to estimate.
  4. For each chosen task, runs `claude -p` with a prompt that demands
     the task be done CORRECTLY, with DOCUMENTATION and TESTING, and
     captures the per-task cost / tokens / duration that Claude reports.
  5. Displays per-task results and a rolled-up cost estimate.

This is a forecasting / planning aid, not a quote. It uses Claude Code's
own cost reporting (`total_cost_usd` from `--output-format json`).
...
# ---------------------------------------------------------------------------
# Theme: Claude — amber/orange brand
# ---------------------------------------------------------------------------

ACCENT_1 = "#F59E0B"   # amber
ACCENT_2 = "#EA580C"   # deep orange
ACCENT_GLOW = "#FB923C"
TOOL_TITLE = "Claude Code Cost Dashboard"
TOOL_TAGLINE = "Forecast real spend on Anthropic's Claude Code CLI"
TOOL_EMOJI = "🟠"
HEADER_CREDIT_HTML = (
    'Created by '
...
def _build_header(parent: QWidget) -> QFrame:
    ...
    _drop_shadow(frame, ACCENT_2, blur=40, offset=6)
    return frame
...
def claude_on_path() -> Optional[str]:
    """Return absolute path to `claude` (or its npm shim) if found, else None."""
    return shutil.which("claude")


def _resolve_claude_invocation() -> Optional[List[str]]:
    """
    Return the argv prefix to launch Claude Code WITHOUT going through the
    Windows .CMD shim (which mangles multi-line argv via cmd.exe re-quoting).

    On Windows, prefers ['<node.exe>', '<...>\\cli.js']. Falls back to the
    plain `claude` exe on Unix.
    """
    exe = claude_on_path()
    if not exe:
        return None

    # Unix: claude is a real script with a shebang, no shim issue.
    if os.name != "nt":
        return [exe]

    # Windows: locate cli.js next to the npm shim and call node directly.
    shim_dir = Path(exe).parent
    cli_js = shim_dir / "node_modules" / "@anthropic-ai" / "claude-code" / "cli.js"
    node_exe = shutil.which("node")
    if cli_js.is_file() and node_exe:
        return [node_exe, str(cli_js)]
...
def run_claude(
    ...
    """
    Run `claude -p` once. Returns (parsed_json, error_string, duration_sec).
    """
    invocation = _resolve_claude_invocation()
    if not invocation:
        return None, "`claude` not found on PATH — install Claude Code first", 0.0
    ...
    # If we had to fall back to the .CMD shim, we must use shell=True.
    use_shell = (
        os.name == "nt"
    ...
    try:
        returncode, stdout, stderr, duration, run_error = _run_cli(
            cmd, repo_path, timeout, use_shell=use_shell,
    ...
    try:
        return json.loads(stdout), None, duration
    except json.JSONDecodeError as e:
        return None, f"json decode failed: {e}", duration
...
def extract_cost(data: dict) -> TaskCost:
    ...
    tc.cost_usd = float(data.get("total_cost_usd") or data.get("cost_usd") or 0.0)
...
class SuggestWorker(QThread):
    finished_ok = pyqtSignal(list, dict)        # tasks, raw_cost_dict
    ...
    def run(self) -> None:
        try:
            self.log.emit(f"Asking Claude to analyze {self.repo} ...")
            data, err, dur = run_claude(
            ...
            # Claude returns its answer as a string in `result` (Claude Code JSON
            # envelope). Fall back to other common keys just in case.
            text = (
                data.get("result")
            ...
            if isinstance(text, list):
                # Some versions return content as a list of {type,text} chunks
                text = "".join(
            ...
            if lb == -1 or rb == -1 or rb < lb:
                self.failed.emit(
                    "Could not locate a JSON array in Claude's reply. "
                    "Raw reply:\n" + text[:500]
                )
                return
            try:
            ...
            if not suggestions:
                self.failed.emit("Claude returned no usable task suggestions.")
                return
            ...
            self.log.emit(
                ...
                f"(analysis cost ${analyze_cost.cost_usd:.4f}, {dur:.1f}s)."
            )
...
class EstimateWorker(QThread):
    one_done = pyqtSignal(int, object)   # index, TaskCost
    all_done = pyqtSignal()
    log = pyqtSignal(str)
...
class TaskRow(QWidget):
    """One suggested task: checkbox + name + description + weight spin."""
...
class Dashboard(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Claude Code Cost Dashboard")
    ...
    def _build_ui(self) -> None:
        ...
        # Top: repo picker + model + team controls
        ...
        self.model_combo.setCurrentText("opus")
        ...
        # Splitter: tasks (left) | results+log (right)
        ...
        # Right side: results table + summary + log
        right = QWidget()
        right_layout = QVBoxLayout(right)

        results_box = QGroupBox("Per-task cost")
        rb = QVBoxLayout(results_box)
        self.results_table = QTableWidget(0, 5)
        self.results_table.setHorizontalHeaderLabels(
            ["Task", "Cost (USD)", "Tokens", "Turns", "Time (s)"]
        )
        ...
        # 3D drop-shadows on key panels
        for w in (top, tasks_box, results_box, summary_box, log_box):
            _drop_shadow(w, "#000000", blur=24, offset=4)
        _drop_shadow(self.analyze_btn, ACCENT_2, blur=22, offset=3)
        _drop_shadow(self.estimate_btn, ACCENT_2, blur=22, offset=3)
    ...
    def _check_claude(self) -> None:
        path = claude_on_path()
        if path:
            self.statusBar().showMessage(f"claude CLI: {path}")
        else:
            self.statusBar().showMessage(
                "WARNING: `claude` not found on PATH — install Claude Code first."
            )
    ...
    def _on_analyze(self) -> None:
        if not claude_on_path():
            QMessageBox.critical(
                ...
                "Install Claude Code and authenticate it before using "
                "this dashboard.",
            )
            return
    ...
    def _on_suggest_ok(self, suggestions: list, analysis: dict) -> None:
        ...
        self._log(
            f"Repo analysis cost: ${self.analysis_cost:.4f} "
    ...
    def _render_summary(self) -> None:
        ...
        if not valid:
            ...
            not receive Claude's <code style="color:{ACCENT_1};">total_cost_usd</code>
            for that task.</p>
            <p style="color:#94A3B8;">For large repositories, use this analysis
            cost as the confirmed spend and run a smaller/lighter task estimate
            if you need a completed per-task cost.</p>
            """.replace("{ACCENT_1}", ACCENT_1)
            self.summary_label.setText(html)
            return

        # Map results back to chosen tasks (ordered) so we can use weights.
        weight_by_name = {
        ...
        seats_yr = 100 * devs * 12  # Premium @ $100/seat/mo, annual billing
        ...
        <tr><td>Premium seats list price:</td>
        ...
        <p style="color:#94A3B8;">Costs come from Claude Code's own
        <code style="color:{ACCENT_1};">total_cost_usd</code> field. Each task
        was run with a prompt that demanded the change be done correctly with
        documentation and testing — this is meant to reflect <i>real</i>
        per-task spend, not a quick sketch. Real bills can still run 1.5–3×
        higher due to iteration, long autonomous sessions, and MCP context
        bloat.</p>
